...
void printPoint(char output_file[]){
    /*
...
int main() {
    ...
    char* clusteringFileName="/parallelClusteringRectangular.csv";

    char description[1000];
    char* currentPath=getcwd(description, sizeof(description));
    char path_points[1000], path_centroids[1000],path_output[1000];
    char * pathFileCentroids=strcpy(path_points, currentPath);
    char* pathFilePoints=strcpy(path_centroids, currentPath);
    char* pathOutputFile=strcpy(path_output, currentPath);
    pathFileCentroids=strcat(pathFileCentroids,initialCentroidsFileName);
    pathFilePoints=strcat(pathFilePoints,pointsFileName);
    pathOutputFile=strcat(pathOutputFile,clusteringFileName );
    printf("CSV paths: %s, %s, %s\n", pathFileCentroids, pathFilePoints, pathOutputFile);
